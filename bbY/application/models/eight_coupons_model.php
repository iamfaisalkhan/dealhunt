...
class Eight_coupons_model extends CI_Model {
   ...
   /**
    * 
    * @param string $filter filter source_ids <= to this value. 
    * @return string|boolean|multitype:multitype:string NULL
    */
   public function getrealtimeproductdeals($filter = FALSE)
   {
      ...
      foreach($response as $el)
      {
         if ($filter != FALSE)
         {
            $source_id = intval($el->ID);
            if ($source_id <= $filter) continue;
         }
         
         //TODO: check the existance of these properties before returning them
         // from this method. 
         $result[] = array("txt" => "$el->dealTitle",
               "source_name" => "$el->dealSource",
               "source_id" => $el->ID,
               "date_posted" => $el->postDate,
               "date_expired" => $el->expirationDate,
               "url" => $el->URL,
               "image" => $el->showImage
         );
      }
